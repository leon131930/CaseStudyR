...
ggplot(data = patientinfo, aes(x=confirmed_date, y=daily_cases)) + geom_line(colour="grey") + geom_line(data = subset(patientinfo, infection_case=="Itaewon Clubs"), colour="green") + geom_line(data = subset(patientinfo, infection_case=="Richway"), colour="blue") + geom_line(data = subset(patientinfo, infection_case=="Guro-gu Call Center"), colour="purple") + geom_line(data = subset(patientinfo, infection_case=="Shincheonji Church"), colour="red") + geom_line(data = subset(patientinfo, infection_case=="Coupang Logistics Center"), colour="black") + labs(x="Time", y="Cases per Day") + geom_hline(yintercept=100, color="orange")
