...
* Germany on the other hand struggled with a multiple of daily cases and the high numbers remained on a high level for a longer time period before dropping again. 
* Considering that both countries Germany and South Korea are fairly similar in terms of the population of each country(~50 Mio. vs. 80 Mio.), it thus seems interesting to further investigate what happened in South Korea. 
...
## Claim 01 
#### By imposing a multitude of different types of policies, the government of South Korea could reduce the number of daily infection cases.

**Our initial motivation:**

* As the number of infection cases in South Korea increased heavily, the government became more active in terms of new restricting policies across all areas of life with the aim to decrease the number of daily infection cases. 

**Claim description in more detail:**

* The government of South Korea imposed different types of polices across all areas of life.
...
* Then, we show an example of imposed policies in the areas administration, education and social area.
...
plot_nbPolicies <- ggplot(nbPolicies, aes(x = reorder(type, N) ,y = N)) +
  ...
  coord_flip() +
  labs(x = "Type of Policy", y = "Number of Policies",
...
ggplot(time_copy, aes(x=confirmed_date, y=daily_cases)) + geom_line() +
  ...
  labs(x = "Time", y = "Cases per Day",
       title="Administrative / Education / Social Policies")+
  theme(legend.position="none") 
...
* After the number of daily cases have reached peak in March, numerous policies with the aim of social distancing, such as delaying school start, were introduced. It seems that through these policies the number of daily cases could be reduced.
...
#Cross-correlation
ccf(merge_CasesMeasures$cumulativeNumbers,merge_CasesMeasures$daily_cases, 50, main = NA)
```
...
average value of policies is likely to lead to a below average value of daily cases 
about 30 days later. 
* And, a below average of policies is associated with a likely above average daily cases value about 6 months later.
...
**Discussion:**
* Due to the fact, that there are multiple active policies in the same time period and also across different areas it is not possible to reduce the effect of declining number of daily cases to one or even a type of policies. 
* However, it can be said that it is very likly that the different polices from the government had an influence on the decrease in numbers. 
...
But could there by an additional confounding variable distorting this result?
Considering there have been superspreader events in the beginning of the pandemic, 


Given t
We finally investigate a possible confounding variable
