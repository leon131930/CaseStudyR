...
* Therefore, we first inspect the Pearson correlation between both variables before having a closer look at the cross-correlation to gain further insights.
...
* For a lag of zero we obtain the same results as already shown in our test above.
* However, as can be seen, the negative correlation increases from ~ -0.2 at lag 0 up to ~ -0.6 at lag -33.
* This indicates that increased number of policies are likely to lead to decreased number of daily cases about 30 days later.  
...
* Our results clearly show, by only considering these two variables, that one can assume a moderate negative correlation for a time lag of ~ 1 month.  
