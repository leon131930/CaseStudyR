...
south_korea <- raster::getData("GADM", country = "South Korea", level = 1)
...
* Both countries Germany and South Korea are considered as industry nations, thus we can assume that there is at least a small similarity in terms of life quality, infrastructure, health care system, etc. However, South Korea has more than twice the number of inhabitants per km² and still showed low numbers of infection cases. Therefore, it seems interesting to further investigate what happened in South Korea. 
...
## Claim 01 
#### By imposing a multitude of different types of policies, the government of South Korea managed to reduce the number of daily infection cases.

**Our Initial Motivation:**

* South Korea generally showed  covid cases of much lower level (compared to Germany). In addition, South Korea was able to drastically reduce its number of daily covid cases from its peak end of February. As social-distancing policies are known to be a global measure to contain the spread of the virus, we investigate their influence of policies on the course of the pandemic in South Korea.
...
* First, we inspect the different types of policies and when they were imposed by the government during the period from January to July 2020. 
* Then, we show an example of imposed policies in the areas of administration, education and society to outline the overlap of different policies within the same time period. 
...
* In total, 61 policies were introduced by the government of South Korea during the time period from January to June 2020.

* The number of immigration and education policies dominate other policies. This is due to the fact that immigration policies are counted for each country and education policies for each school type.

* Shortly after the number of infection cases surged the government of South Korea reacted with numerous policies in all kinds of areas. Many policies in the middle of April result from online-school opening policies that all took place at once for each type of school.
...
ggplot(time_copy, aes(x=confirmed_date, y=daily_cases)) + geom_line() +
  ...
  labs(x = "Time", y = "Cases per Day",
       title="Start/End of Administrative, Education & Social Policies")+
  theme(legend.position="none") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %y")
...
* After the number of daily cases have reached peak end of February, numerous policies with the aim of social distancing, such as delaying school start, were introduced. It seems that these policies helped reducding the number of daily cases.
(Note: the figure above only displays a fraction of policies)
* In the beginning of May, after the number of daily cases tended to increase again, further public indoor gathering places were forced to close.
...
ggplot(merge_CasesMeasures, aes(x = date)) +
  ...
  labs(title="Daily Cases vs. Number of Policies", x = "Time") +
...
* The number of active policies increased heavily when the number of daily cases rose to its peak end of February and kept increasing until June
* However, as the figure shows, some policies were also removed again.  
* In the following, we inspect the Pearson correlation between both variables number of active policies and daily infection cases.
...
* We presume that there is no correlation between the variables "Daily Cases" and "Number of Policies" (null hypothesis) and test on the common significance level of alpha = 0.05. As the direction of correlation is unknown, we use the double-sided hypothesis test.
* As result we get a Pearson correlation of -0.2 which indicates a low negative correlation between "Daily Cases" and "Number of Policies", meaning that more policies result in lower number of daily cases.
...
* Our results clearly show, by only considering these two variables, that one can assume a moderate negative correlation
* However, we need to take into account that there are multiple active policies in the same time period and also across different areas. The first two figures presented in this claim show the type of different introduced policies and their start dates while the following figure examplifies the overlap of various polices. Therefore, it is not possible to reduce the effect of declining number of daily cases to one or even a type of policies. 
* Furthermore, there is need to consider various other possible impacts that could have randomly caused this relationship.
* Nevertheless, it can be said that it is very likely that the different polices from the government had an influence on the decrease in numbers of daily infection cases.


<p>&nbsp;</p>

## Claim 02
#### The superspreader event in the Shincheonji Church in Feb. 2020 was one reason the Covid cases in South Korea turned out to be so intense in the first place.

**Our Initial Motivation:**

* When plotting overall daily cases in South Korea over time one clearly sees that during the first couple of weeks the number of confirmed Covid cases remained relatively low. However, at some point in late February they skyrocketed.
* We wanted to dive deeper into this "turning point". Since we have heard on the news multiple times that a lot of people worldwide including the local government blamed a Christian "cult" for the explosion of cases, we decided to take a closer look at this event in particular.

**Claim Description in More Detail:**

* We are claiming, that without this particular spreader event in the Shincheonji Church in Daegu the Covid-spread would have been seriously lower.
...
* In order to see, whether the public media was right, and the church did indeed cause the first "serious" spread of the virus we first had a look at the Covid infections throughout South Korea.
* Following we had a closer look at the church cases compared to the overall daily cases and dived a bit deeper into the starting point of the church cluster.
* Finally, we analyzed if this local event actually effected the whole country. Therefore we plotted a map showing all the people that according to our data were related to the church but were tested positive for Covid in a different region than Daegu. 

We start our analysis by first looking at the spread of Covid cases throughout the country. Where there some areas the cases were really high or was it a more "even" spread?
...
map_overall_cases <- ggplot() +
  ...
  coord_map() +
  labs(title = "Confirmed Covid-Cases in South Korea", x="Longitude", y="Latitude")
...
map_overall_cases_final <- ggplot() +
  geom_polygon(data = south_korea.f, aes(x = long, y = lat, group = group), fill = "lightgrey",
               colour = "grey", size = 0.25) +
  geom_point(data = case_02_withcoordinates, aes(x=longitude, y=latitude, size = confirmed),
             color = "violetred",
...
```{r  echo=FALSE}
map_overall_cases_final
```
 <p>&nbsp;</p>
**Short Analysis:**

* Seems like there were two hotspots indeed! One around Seoul and one around the mysterious church event.
* Let's have a closer look at this. How did the church related cases develop over time and did they have a significant impact on the overall daily cases?
...
church_cases_overtime <- ggplot() + geom_line(data = patientinfo_02,
                     aes(x=confirmed_date, y=daily_cases, colour="Overall Daily Cases")) +
  geom_line(data = patientinfo_02_church,
            aes(x=confirmed_date, y= dailychurchcases, colour = "Shincheonji Church related Cases")) +
...
* At first it doesn't seem like the Shincheonji cases were such a big deal. Since compared to the overall daily cases the number of church related cases was relatively low.
* But when we think about the fact that this is only one single event, the numbers don't seem to be that low at all.
* Still, this cannot be the reason everyone was so convinced the Shincheonji church was the bad guy. Maybe the dataset (= the patientinfo datatable) we used isn't showing the whole picture?
* Let's dive a bit deeper. There has to be a reason why everyone was going nuts about this one! Are we able find the top 10 infection reasons? Let's have a look at the case datatable, since it provides approximately two times as many datapoints, as the patientinfo datatable does. 
...
* Let's have a closer look at the beginning of the Shincheonji Church case-outbreak. How did it start? Was there a certain "trigger"?
* Let's look for the very first patient that was confirmed positive for Covid **and** was related to the Shincheonji Church. In the news they were talking about an elderly women... Can this be confirmed with the help of our dataset?
...
```
<p>&nbsp;</p>
* Turns out in fact patient number 31, in her 60s, was diagnosed with Covid on Tuesday, 18th Feb 2020, as the first person related to the Shincheonji Church and she has an **insane** amount of contact numbers.
* No wonder Shincheonji Church cases were so high... Imagine those 1160 contacts infecting even more people!
* Let's quickly plot this over time to get the whole picture and see on what day patient 31 was going to the church and at what point in time she was actually diagnosed. 
<p>&nbsp;</p>
...
* This fits the timeline and development of overall cases pretty well. 
* However, it still doesn't provide a clear picture why the church was blamed for the virus to go wild, does it?
...
* Is there a way to determine, whether people that were infected with Covid at this particular church event did travel to a different city or region, basically "taking the virus with them"?
...
* Alright, indeed a couple of people were tested positive in different regions than Daegu but were still identified as "Shincheonji Church related". Thus indicating, that they travelled while being infected with the virus. 
* Gyeongsangbuk-do has quite a large number of positive tested people that are related to the church.
* Are we able to plot this on a map showing in what region exactly the church related people were tested positive for Covid-19?
...
* While obviously the most cases related to the church occurred in Daegu or close to Daegu, this map shows pretty well that positive tested people that were somehow related to that church event, appeared **all over the country**. Important to mention here is, that this weren't just a few but numerous incidents sometimes amounting to hundreds of infected people. 
* Thus, people traveling from Daegu to other regions within South Korea while being infected, might have caused the virus to spread: from that one outbreak in Daegu into the whole country - and maybe even further.

<p>&nbsp;</p>

**Discussion:**

* It seems like the event at the church indeed had significant effects on the Covid spread throughout South Korea.
* This highlights the importance of policies such as travel restrictions, that are making sure that in case such an event happens, the infected people do not travel to a different location, taking the virus with them. 
* However, one cannot say for sure, that the church was the main reason for the severe Covid outbreak in South Korea. There are simply too many other variables that could have influenced this. For one, there were multiple spreader events, not only the church one (see the bar chart "Top 10 Infection Reasons").
* Additionally, we do not know for sure, that patient number 31 was the only person infected at that church event. While she was the first one to be tested positive we do not know whether there was a 2nd, 3rd or 4th person infected at that church event that simply wasn't tested positive or didn't get tested at all.

* To sum it up, YES, the church event had an impact on the Covid spread in South Korea. We can confirm, that the Shincheonji Church event was one reason that ultimately led to skyrocketing cases in the country. Especially the fact that people that got infected at the church event traveled to different regions caused the Shincheonji Church to not only have a local but a nation-wide impact. And thus, we are able confirm our claim. 
* However, we would **not** be able to confirm that the superspreader event was one of the main reasons for the outbreak. So the picture that was communicated in the local as well as international media may not be accurate.
<p>&nbsp;</p>


## Claim 03
#### The temperature level correlates with daily number of infection cases.

**Our Initial Motivation:**

* Concerning other viruses it is often said that the temperature level has a significant impact on the daily number of infection cases.
Thus it is reasonable to inspect whether this is also true for COVID-19.

**Claim Description in More Detail:**

* Therefore, we analyse the association between the average temperature level and the number of covid cases per day within the period from January to June 2020. Thereby, we look at the data of the different provinces of South Korea. Within the time period from January to June 2020 the average temperature ranked between -8℃ and 30℃.  

**Our Approach:**

* First, we plot the existing data as scatter plot in order to see the kind of underlying distribution. For this, we plot the average temperature on the x-axis and the number of daily infection cases on the y-axis. By inspecting the plot we then can decide whether to use a Pearson or Spearman test. Depending on the results of the test, we are able to reject or fail to reject our hypothesis. As we want to show that temperature level associates with daily number of infection cases our null hypothesis formulates as follows: 
Ho : r = 0. We use the commonly used significance level of alpha = 0.05, meaning that we will reject Ho if the p-value is below 0.05.
...
ggplot(melted_dt,aes(avg_temp,daily_cases,color=ifelse(province=='Daegu', "Daegu", "Remaining Provinces")))+geom_point() +
  labs(x = "Average Temperature in °C", y = "Cases per Day", 
...
* As can be seen above, the province Daegu has way higher numbers of daily infection cases and thus distorts the entire plot which is a typical characteristic for outlier. Therefore, it seems to be reasonable to plot the data without this province in order to get a better understanding on the relationship between temperature and number of daily infection cases in the remaining provinces. 
* Nevertheless, we already can conclude that the data is not normally distributed. Also, the Spearman correlation test is more robust to outliers than the Pearson correlation test. Thus, we prefer to apply the Spearman correlation test.
...
ggplot(melted_dt_without_Daegu,aes(avg_temp,daily_cases))+geom_point() +
  ...
  labs(x = "Average Temperature in °C", y = "Cases per Day", 
       title = "Relation: Avg. Temperature vs. Daily Cases (without Daegu Province)") +
  theme(legend.title=element_blank()) 
...
* Without the data from the province Daegu it is possible to rectify the data points. However, no considerable correlation between the two variables can be seen.
...
* It is important to note that we only fail to reject the null hypothesis. We cannot say that the number of infection cases is independent of the temperature level.
* Other circumstances, like super spreader events, distort the number of infection cases for individual regions and thus may obscure a possible association between temperature level and number of infection cases.


<p>&nbsp;</p>

## Claim 04
#### Search data is associated with the development of daily cases
...
* As large-scale covid testing was not yet implemented in the beginning of the pandemic, infected individuals may not have been tested immediately after the onset of covid symptoms.
* Considering South Koreas advanced level of digitization, it is probable that infected individuals searched for covid-related symptoms online after their onset of symptoms.
* It should thus be explored whether search data is related to covid-cases, with a respective time gap between individuals searching online for symptoms and eventually being tested.

**Our Approach:**

* To explore this possible relation, we first select available search data - as the search volume for different keywords is in relative units (preventing volume comparisons), we choose the keyword "coronavirus".
* As time period for investigating the relation between searches for "coronavirus" and daily cases we select with Mid-February to end of June.
* As data foundation for daily cases we use the "Time"-table which contains more data than the table "Patientinfo".
* After joining both tables, we plot the data and calculate the correlation between both variables. We also look into cross-correlation to account for a possible time gap between search volume and daily cases.
* Finally we apply the Pearson correlation test to possibly reject the null-hypothesis of not correlated variables upon the commonly used significance level of alpha = 0.05.
...
ggplot(merged_dt1[date %between% c("2020-02-15", "2020-06-29")],aes(x=date)) +
  ...
  geom_line(aes(y=daily_cases/8, col = "Cases per Day")) +
  scale_color_manual(values=c(dailycasecolour,searchcolour))+
  labs(x="Time", title="Cases per Day & relative Search Volume for 'coronavirus'")+
  scale_y_continuous(
    name = "Relative Search Volume",
    sec.axis = sec_axis(trans=~.*8, name="Cases per Day",labels = scales::comma))+
  theme(legend.position="bottom", legend.title = element_blank(), 
        axis.title.y.right = element_text(angle=90, color = dailycasecolour),
        axis.title.y.left = element_text(color = searchcolour))
```
* Above plot shows the relative search volume of the term "coronavirus" and the daily cases by time. To account for the different units of both variables, we use a second y-axis.
...
```
* Referring to the time-lag described in our motivation of this claim, we calculate the cross-correlation:
...
* But could there be an additional confounding variable distorting above result?
...
* Our approach is similar as before: We plot both variables (now cases of Shincheonji church and total cases per day) and calculate the Pearson correlation.
...
As the covid outbreak in Shincheonji church was a one-time event, we only take confirmed cases until ten days after the event (18.02-28.02.) into our calculation:
...
ggplot(merged_dt2[date %between% c("2020-02-15", "2020-03-08")],aes(x=date)) +
  ...
  geom_line(aes(y=daily_cases/20, col = "Cases per Day")) +
  geom_line(aes(y=(dailychurchcases), col = "Shincheonji Church Cases per Day")) +
  scale_color_manual(values=c(dailycasecolour, churchcolour))+
  labs(x="Time", title="Shincheonji Church Daily Cases vs. Total Cases per Day")+
  scale_y_continuous(
    name = "Shincheonji Church Cases per Day",
    sec.axis = sec_axis(trans=~.*20, name="Cases per Day",labels = scales::comma))+
  theme(legend.position="bottom", legend.title = element_blank(), 
        axis.title.y.right = element_text(angle=90, color = dailycasecolour),
...
* Above results show no significant association of both variables; we cannot reject the null hypothesis of not correlated data.


**Discussion:**

* Overall, the available search data and its correlation to covid cases seems to be a promising factor to be applied for predictions of covid cases.
* However, as South Korea quickly accelerated its testing capacities, our calculated time gap of 6 days might not be valid under those circumstances.

* Also, it is worth mentioning that Google previously tried simulating infectious diseases like the flu based on Google Search data, which turned out to be unsuccessful.
* Therefore one should be very careful in generalizing our findings.
* Further research on this should be done for countries with a second wave of covid cases in a setup of well-developed testing capacities.
...
* Policy measures imposed to contain the virus were most likely successful
    * Due to a variety of different policies, the effect of individuals policies on daily           cases could not be shown

* Superspreader events are indicative of accelerating the course of the covid pandemic
    * The effect of the church-event on overall cases could not be robustly proven due to a         lack of data
    * Additional data on superspreader events to be analyzed

* Average temperature does not significantly correlate with daily cases
    * Specific in-door temperature data for large gatherings could be subject of further            research 


* Search data can possibly predict future covid-outbreaks, considering the time lag & strong positive cross-correlation
    * Timely limited data was used (one ‘wave’ of cases)
    * More countries & covid waves need to be analyzed in order to generalize the predictive        character of search data
