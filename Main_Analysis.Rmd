...
* Therefore, we analyse the association between the average temperature level and the number of covid cases per day within the period from January to June 2020. Thereby, we consider the daily cases and average temperature of the different provinces of South Korea. Within the time period from January to June 2020 the average temperature ranked between -8℃ and 30℃.  

**Our approach:**

* First, we plot the existing data into a scatter plot in order to see the kind of underlying distribution. For this, we plot the average temperature on the x-axis and the number of daily infection cases on the y-axis. By inspecting the plot we then can decide whether to use a Pearson or Spearman test. Depending on the results of the test, we are able to reject or fail to reject our claim. As we want to show that temperature level associates with daily number of infection cases our null hypothesis formulates as follows: 
...
* It is important to note that we only fail to reject the null hypothesis. We cannot say that the number of infection cases are independent of the temperatur level.
* Other circumstances, like super spreader events, distort the number of infection cases and thus may obscure a possible association between temperature level and number of infection cases.
